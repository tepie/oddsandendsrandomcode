            ...
            if not user_model == None:
                ...
                if user_model.access_token_key and user_model.access_token_secret:
                    ...
                    try:
                        ...
                        #search_results = memcache.get("search_results:%s" % user.user_id())
                        #if search_results == None:
                        #    search_q = "#twixmit" # from:%s" % twitter_user.screen_name
                        #    search_results = api.search(q=search_q,result_type="recent")  
                        #    memcache.add("search_results:%s" % user.user_id(), search_results, 60)
                        
                        
                        #_template_values["search_results"] = search_results
